...
using  Unitful, UnitfulRecipes, Plots, PlotlyJS, ColorSchemes
...
# ╔═╡ 9da8b652-11d5-4239-abb2-b165f89c7a51
...
{ .center .w-50 title="Textbook Figure P3.25" }
...
1. **The units requested are kJ/kg, so we need to find _mass specific_{.killerorange} work,
   ``W/m``.**

1. Let _...the piston moves smoothly in the cylinder_ scream to you:
   This is an **isobaric process**{.killerorange}!!  
...
# ╔═╡ 76529ec7-c678-4569-b121-8391b22b43a1
...
{.message .danger} 
> actually, **do not round** if your property pair is ``P, T`` and you're testing for saturation conditions. A ``(T_{sat},P_{sat})`` combination must be exact.

 - From `Table A-11`{.mcs .highlight .property-table}, 
   ``v_1 = v_{\rm g@16bar} = `` $(v1 |> u"m^3/kg" |> ustrip) m³/kg

 - From `Table A-12`{.mcs .highlight .property-table}, 
   ``v_2 = v_{\rm 16bar,160°C} = `` $(v2 |> u"m^3/kg" |> ustrip) m³/kg
...
begin
	...
	Psat_r134a = [0.5164,0.6332,0.7704,0.9305,1.0199,1.1160,1.2192,1.3299,1.4483,1.5748,1.8540,2.1704,2.5274,2.9282,3.3765,3.8756,4.4294,5.0416,5.7160,6.4566,6.8530,7.2675,7.7006,8.1528,8.6247,9.1168,9.6298,10.164,10.720,11.299,12.526,13.851,15.278,16.813,21.162,26.324,32.435,39.742]u"bar"
	
	vf_r134a = [0.7055e-03,0.7113e-03,0.7172e-03,0.7233e-03,0.7265e-03,0.7296e-03,0.7328e-03,0.7361e-03,0.7395e-03,0.7428e-03,0.7498e-03,0.7569e-03,0.7644e-03,0.7721e-03,0.7801e-03,0.7884e-03,0.7971e-03,0.8062e-03,0.8157e-03,0.8257e-03,0.8309e-03,0.8362e-03,0.8417e-03,0.8473e-03,0.8530e-03,0.8590e-03,0.8651e-03,0.8714e-03,0.8780e-03,0.8847e-03,0.8989e-03,0.9142e-03,0.9308e-03,0.9488e-03,1.0027e-03,1.0766e-03,1.1949e-03,1.5443e-03]u"m^3/kg"
	
	vg_r134a = [0.3569,0.2947,0.2451,0.2052,0.1882,0.1728,0.1590,0.1464,0.1350,0.1247,0.1068,0.0919,0.0794,0.0689,0.0600,0.0525,0.0460,0.0405,0.0358,0.0317,0.0298,0.0281,0.0265,0.0250,0.0236,0.0223,0.0210,0.0199,0.0188,0.0177,0.0159,0.0142,0.0127,0.0114,0.0086,0.0064,0.0046,0.0027]u"m^3/kg"
...
# ╔═╡ b1449db0-c818-4fb5-9d72-78936db99cbe
begin
	# ═══ load css ════════════════════════════════════════════*═
	
	
	const gentleblue            = colorant"#5e81ac"
	const troublered            = colorant"#ff2600"
	const killerorange          = colorant"#db5728"
	const wellhellothereorange  = colorant"#f7800f"
	const addblue               = colorant"#037db6"
	const ayustringgreen        = colorant"#86b300"
	const onsomeothergreen      = colorant"#439d00"
	const linkgreen             = colorant"#008800"
	const audiblegreen          = colorant"#00f900"
	const tiger 				= colorant"#411ff5"
	const chartreuse 			= colorant"#c1f80a"
	const cyan 			        = colorant"#00ffff"
	const hyabusabright 		= colorant"#c3eaff"
...
# ╔═╡ 6b26235d-d148-4dd5-a1ad-13acea9a878a
#=

Monitors system darkmode status
...
begin
	...
	<span>
		<script>
			...
			mediaQueryList.addEventListener('change', (e) => {
				span.value = mediaQueryList.matches;
	...
	<p class="appendix-item dull">
		<span class="newthought">infrastructure</span>
		&emsp; realtime system darkmode monitor
	</p>
...
# ╔═╡ 12524387-5a27-499b-9933-8c4f0522b425
begin

  # substitue dark mode color(s)
	colered = nthColor(darkmode, ColorSchemes.julialegacy, 3, audiblegreen)
	
	# theme(:default)
	process_color = colered[3]

  # choose Plotly.js as the plotting engine
	plotlyjs()

	
  # the annotations routines don't do Unitful quantities,
  # so we strip off the units now to keep our code cleaner later
    (_v1, _v2, _P) = (v1, v2, P) .|> ustrip

  # saturation dome
    Plots.plot([vf_r134a, vg_r134a], Psat_r134a, 
		palette = palette(colered),
		lab = ["sat.liquid" "sat.vapor"],
		linewidth = 1,
	)

  # the process
	Plots.plot!([v1, v2], [P, P], 
		linewidth = 2,
		label  = "process",

		marker = :dot,
		markersize = 3,
		markerstrokecolor = process_color,
		
		annotations = [
			(_v1, _P, Plots.text("1", :right, :bottom, process_color, 12)),
			(_v2, _P, Plots.text("2", :left , :bottom, process_color, 12)),	
			((_v1+_v2)/2, _P/2, Plots.text("work", :center, process_color, 12)),
		],
	...
	@mdx("""
	{.appendix-item}
	_plot_{.newthought .appendix} &emsp; The process and its associated work
	""")	
end
